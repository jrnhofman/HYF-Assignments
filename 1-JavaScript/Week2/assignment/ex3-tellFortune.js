...
export function tellFortune(numKids, partnerNames, locations, jobTitles) {
  const j = selectRandomly(jobTitles);
  const n = selectRandomly(numKids);
  const p = selectRandomly(partnerNames);
  const l = selectRandomly(locations);
  return `You will be a ${j} in ${l}, married to ${p} with ${n} kids.`;
}
